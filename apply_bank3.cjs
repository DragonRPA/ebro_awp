...
const target = `<th style={{ padding: '8px 10px', whiteSpace: 'nowrap', width: '150px', position: 'sticky', left: 0, zIndex: 2, backgroundColor: 'var(--bg-main)' }}>관리/액션</th>
                  <th style={{ padding: '8px 10px', whiteSpace: 'nowrap' }}>분류</th>
                  <th style={{ padding: '8px 10px', whiteSpace: 'nowrap' }}>거래일시</th>
                  <th style={{ padding: '8px 10px', whiteSpace: 'nowrap' }}>은행</th>
                  <th style={{ padding: '8px 10px', whiteSpace: 'nowrap' }}>적요내용 (계좌번호/거래처)</th>
                  <th style={{ padding: '8px 10px', whiteSpace: 'nowrap', textAlign: 'right' }}>입금액 [＋]</th>
                  <th style={{ padding: '8px 10px', whiteSpace: 'nowrap', textAlign: 'right' }}>출금액 [－]</th>
                  <th style={{ padding: '8px 10px', whiteSpace: 'nowrap', textAlign: 'right' }}>거래 후 잔액</th>
                  <th style={{ padding: '8px 10px', whiteSpace: 'nowrap' }}>매칭/정산처</th>
                  <th style={{ padding: '8px 10px', whiteSpace: 'nowrap' }}>매칭 상태 (청구/매입)</th>`;
...
if (content.includes(target)) {
  ...
  const startIdx = lines.findIndex(l => l.includes("관리/액션</th>"));
  if (startIdx !== -1) {
