...
const ApprovalInbox: React.FC = () => {
  ...
  const handleAction = async (stepId: string, action: 'APPROVED' | 'REJECTED') => {
    if (!supabase) return alert('DB 연결 오류');
          ...
          {requests.map(r => {
            ...
            return (
              <tr key={r.id}>
                ...
                <td style={{ padding: '10px', borderBottom: '1px solid #eee', textAlign: 'center' }}>
                  <button onClick={() => handleAction(r.id, 'APPROVED')} disabled={loading} style={{ marginRight: '8px', padding: '4px 12px', backgroundColor: '#3b82f6', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>승인</button>
                  <button onClick={() => handleAction(r.id, 'REJECTED')} disabled={loading} style={{ padding: '4px 12px', backgroundColor: '#ef4444', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>반려</button>
                </td>
              </tr>
            );
          })}
          {requests.length === 0 && (
            <tr><td colSpan={5} style={{ padding: '30px', textAlign: 'center', color: '#888' }}>대기 중인 결재 건이 없습니다.</td></tr>
          )}
        </tbody>
      </table>
    </div>
