...
async function run() {
  const { error } = await supabase.from('vendors').upsert({id: 'TEST', name: 'TEST', type: 'OTHER', tenant_id: 'giyeun', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString()});
  console.log('OTHER type:', error ? error.message : 'success');
}
run();
