...
## [완료] 현장/고객 옵션 마스터 식별 시 'OOO n개' 수량 표기 제거 및 순수 옵션 품목명 정규화 & DB 일괄 클렌징
- **요구사항**:
  "초기DB 업로드 때, 현장별 옵션 을 식별하는 과정에서 OOO n개 라고 식별하고 있어, 이것은 당시의 장비가 n 대 이기 때문에 옵션이 n개 들어가는것인데, 현장별 옵션을 ooo n개 라고 저장해버리는건 오류인것 같아"
- **문제의 본질**:
  - 초기 DB 업로드(출고요청 본문/엑셀) 파싱 시, 당시 투입 장비 대수(n대)에 맞춰 기재된 옵션 수량(예: `협착난간대 * 6`, `감지봉 4EA`, `(4EA)`, `중간발판 3개`)이 현장 및 고객 옵션 마스터(`paidOptions`, `defaultPaidOptions`)에 수량 단위까지 포함되어 영구 저장되는 결함 발생.
  - 이로 인해 이후 신규 계약/출고 시 장비 대수와 무관하게 `협착난간대 * 6`이 상속되거나, 다른 출고 게시글의 `협착난간대 * 2`와 결합되어 `협착난간대 * 6, 협착난간대 * 4, 협착난간대 * 2` 처럼 중복 오염 발생.
  - 현장 옵션의 본질은 "해당 현장에 투입되는 장비마다 장착해야 하는 규격 품목명(협착난간대, 감지봉, 3면 함석, 소화기함 등)"이지, 총 수량이 아님.
- **아키텍처 및 구현 내역**:
  1. **옵션 수량 제거 및 순수 품목명 정규화 엔진 신설 (`src/services/migrationEngine.ts`)**:
     - `cleanOptionItem(raw: string): string`:
       - 괄호 수량 `(4EA)`, `(4)`, `(2개)` 제거 (단, `(소)`, `(GS1930용)` 등 규격/호환 텍스트 보존)
       - 곱하기 수량 `* 6`, `* 1대`, `* 4 ea`, `x 2` 제거
       - 단위 수량 `2개`, `3대`, `4EA`, `1세트`, `2개씩` 제거 (단, `3면`, `4면` 등 면수 및 `20m` 등 길이 보존)
       - 명사 뒤 수량 `감지봉 4`, `과상승방지봉(4)` 제거
       - 명칭 띄어쓰기 표준화 `협착 난간대` ➔ `협착난간대`
     - `normalizeOptionList(rawOptions)`:
       - 콤마/슬래시/배열/JSON 형태의 복수 옵션을 각각 `cleanOptionItem`으로 정제 후, Set을 통해 중복을 100% 제거하여 표준 콤마 문자열로 결합.
  2. **밴드 출고요청 파서 & 분석 엔진 전수 적용 (`migrationEngine.ts`)**:
     - `parseBandDispatchesToPosts`: 유상옵션(`optVal`), 괄호옵션(`extOpt`), 모델란 기재 옵션(`modelVal`), 불릿 라인(`l`)에서 수량 제거 정제 적용 및 반환 시 `normalizeOptionList` 보장.
     - `extractBandDispatchAnalysis`: `combinedPaidOpts` 수집 시 `normalizeOptionList`로 정제하여 중복 완전 차단, `defaultPaidOptions` 및 현장별 `paidOptions` 순수 품목명 정규화.
     - `ingestCustomerDefaultsFromDispatchHistory`: 마스터 DB 동기화 시 기존 DB의 수량 오염 데이터까지 자동 정규화 갱신.
     - 엑셀 마이그레이션: 비고 메모에서 옵션 추출 시 `cleanOptionItem` 적용.
  3. **DB 인터페이스 주석 정정 (`src/services/db.ts`)**:
     - `Customer.defaultPaidOptions` 및 `CustomerSite.paidOptions` 주석에서 잘못된 수량 표기 예시(`협착방지봉 4EA` 등)를 순수 품목명 예시(`협착난간대, 상부덮개, 소화기함`)로 개편.
  4. **기존 Supabase DB 전수 일괄 클렌징 집행 (`scratch/cleanse_all_options.cjs`)**:
     - 원격 Supabase DB의 전체 **현장 356개소, 고객 261개사**에 누적되어 있던 수량 오염(`협착난간대 * 6`, `감지봉 4EA`, `(4EA)`, `중간발판 2개` 등)을 전수 정제 완료.
  5. **단위 테스트 및 빌드 검증 (`src/tests/option_normalization.test.ts`)**:
     - 4개 핵심 시나리오(단일 수량 제거, 규격 보존, 중복 합산, 배열/JSON 처리) 100% PASS.
     - `cmd.exe /c npm run build` 1.07s 무오류 통과.
