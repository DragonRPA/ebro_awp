## [v1.17.1.Build.101] - 2026-09-19 22:10

### 🇰🇷 [전사 UI 표기 순수 한글 단일 표준화 및 수리재투입 전용 별도 UI 제거·화면 간소화]

**배경 및 목적**:
- 대한민국 내 실무 전문가 대상 서비스 원칙에 입각하여, 화면 전체에 불필요하게 병기되던 '한글(영어)' 표기(예: `대여중 (RENTED)`, `출고 (OUTBOUND)`, `재생 대상 (REBUILD)` 등)를 전면 제거하고 직관적인 **순수 한글 단일 표준**으로 통일.
- 장비 회수는 수리 외에도 현장 사정, 규격 변경 등 무수히 많은 이유로 발생하므로, 다시 나가는 시점에는 회사의 가용 재고 여건에 따라 기존 장비든 타 장비든 통상 출고로 배정하는 것이 비즈니스의 자연스러운 본질임. 이에 따라 계약 화면의 불필요한 '수리 장비 재투입' 전용 버튼 및 팝업 모달을 전면 제거하여 화면을 깔끔하게 간소화.
- 전사 표준 헌장 준수:
  - 1.1 최우선 개발 사명 (임직원의 최소 노력으로 최대 편익 창출)
  - 1.2 렌탈 도메인 3대 핵심 가치 (임직원 업무의 최소 조작 & 최상의 편의성)
  - 3.1 무수식어 건조한 명사·동사 UI 단일 표준화 정책

**1. 전사 25개 주요 파일 순수 한글 단일 표준화 완료**:
- 배차 관리 (`Deliveries.tsx`): 출고, 회수, 교환, 이동, 의뢰중, 배차완료, 배송완료, 미정산, 정산완료
- 대시보드 (`Dashboard.tsx`): 최고관리자, 부서관리자, 영업담당자, 정비담당자, 배차물류담당자, 임직원, 대여중
- 자산 관제 (`AgenticAssetLifecyclePage.tsx`): 임대가능, 출고대기, 대여중, 정비중
- 배차 스튜디오 (`AgenticDispatchStudioPage.tsx`): 교환, 출고, 회수
- 소모품 관리 (`ConsumableStockPage.tsx`): 재생 대상, 폐기/고철, 제조사 무상보증, 수거 접수, 처리 진행중, 조치 완료, 작성중, 실사확정, 취소
- 자금흐름 분석 (`CashFlowPage.tsx`): 구간 수납 예정, 구간 운영 지출, 구간 설비 투자, 수납액, 운영지출, 투자지출
- 법인카드 (`CorporateCardPage.tsx`): 기계장치
- 자산 취득/매각 (`AssetAcquisitionDisposal.tsx`): 당사 주기장 상차도
- 계약 관리 (`Contracts.tsx`): 교환 왕복 배차, 거래 불가, 출고금지, 현장간 장비 이동 운송 배차
- 정비 관리 (`Repairs.tsx`): 표준 조치 가이드, 정비중, 임대가능
- 배차 관제 (`TruckDispatch.tsx`): 대차 왕복배차
- 차량 운행 (`VehicleOperationLogPage.tsx`): 경유, 휘발유, 전기
- 제품 마스터 (`Products.tsx`): 피트
- 정기 보고 (`RegularReportsPage.tsx`): 교환 절감, 영업 면제 주의, 영업 청구 면제 손실 투명 보고
- 기타 시스템 배지 및 모달 전수 정돈 (`ErpReadinessBadge.tsx`, `OtApprovalDocumentModal.tsx`, `MobileDispatchOrderCreate.tsx`, `MobileHome.tsx`, `AppContext.tsx` 등)

**2. '수리장비 재투입' 별도 UI 전면 소탕 및 화면 간소화 (`Contracts.tsx`)**:
- 체결 자산 목록 테이블 헤더의 `[수리 장비 재투입]` 버튼 삭제.
- 체결 자산 행별 액션의 `[재투입]` 버튼 삭제.
- `showRedeployModal` 팝업 모달 JSX 및 관련 상태 변수, 이벤트 핸들러 완전 소탕 ➔ UI 정보 밀도 및 전문성 극대화.
...
**배경 및 목적**:
- 사장님 요청: "그리고, 데모 사이트에서는 테넌트 값에 다른 회사, 다른 로고를 사용해서 보여줘야 하는거 아닌가", "e-Bro렌탈 이라고 하자"
- 신규 고객사(A사, B사 등) 미팅 시연 시 기연리프트 명칭과 로고가 노출되어 특정 업체 전용 프로그램으로 오인되는 것을 방지하고, 독립적인 고소작업대 렌탈 표준 ERP 솔루션으로서의 신뢰성과 전문성을 극대화하기 위해 데모 테넌트 브랜딩을 전면 교체.

**주요 조치 내역**:
1. **데모 테넌트 사명 및 사업자 정보 공식 변경**:
   - 상호/법인명: `(주)e-Bro렌탈` (`e-Bro렌탈`)
   - 시스템명: `e-Bro AWP ERP`
   - 사업장 주소: `경기도 화성시 남양읍 화성로 1234 (e-Bro렌탈 화성주기장)`
   - 대표전화: `031-355-8899`, 세금계산서 이메일: `demo@ebro.run`
2. **데모 Supabase DB 및 골든 데이터셋 영구 반영**:
   - 데모 DB(`idfecoovqkjopgbezcpo`) `tenants` 테이블 즉시 업데이트 완료.
   - 골든 데이터셋(`src/services/demo_golden_dataset.json`)의 `tenants` 항목 동기화 완료 (원클릭 리셋 시에도 `(주)e-Bro렌탈`로 영구 보존).
3. **데모 전용 모던 CI 벡터 로고 구축 (`public/images/ci/ebro_rental_ci.svg`)**:
   - 시저 리프트 기하학 모티브의 입체적 엠블럼과 모던 타이포그래피가 결합된 전용 SVG 로고 신설.
4. **도메인별 UI 브랜딩 완벽 동적 분기 (`src/App.tsx`)**:
   - **데모 모드(`awp-demo.ebro.run`)**:
     - 로그인 화면: `ebro_rental_ci.svg` + `(주)e-Bro렌탈` + `e-Bro AWP 고소작업대 ERP` 표출.
     - 메인 헤더: 테넌트 로고 및 상단 회사명을 `(주)e-Bro렌탈` 및 `e-Bro AWP ERP`로 표출.
   - **실운영 모드(`ebro.run`)**:
     - 기존 기연리프트 공식 CI 로고와 `(주)기연리프트` 명칭 100% 무변경 보존.

**검증 결과**:
- 프로덕션 빌드: `npm run build` (`ebro_awp@0.0.0`) 1.02s 무오류 클린 통과.

---

## [v1.16.0.Build.90] - 2026-09-18 23:06

### 🛡️ [기연리프트 실운영 화면 내 시연 데모 버튼/배너 전면 비노출(0% 차단) 및 awp-demo.ebro.run 전용 독립 격리 배포]

**배경 및 문제의식**:
- 사장님 지적: "이 버튼이 기연리프트에 보이면 안되는것 같은데? 기연리프트에서는 싫어할것 같아"
- **현상 및 원인 분석**:
  - `src/App.tsx` 로그인 화면에서 `[⚡ 시연 데모 모드로 체험하기]` 버튼이 도메인 분기 없이 무조건 렌더링되고 있었음.
  - 기연리프트 임직원이 접속하는 실운영 도메인(`ebro.run`, `giyuenlift.ebro.run`)에 데모 버튼이 노출될 경우 고객사 혼란 및 사용자 경험 저해 우려.
  - `src/services/demoMode.ts`의 `isDemoMode()`가 로컬스토리지 잔여 플래그를 참조하여 실운영 도메인에서도 오작동할 수 있는 취약점 존재.

**주요 조치 내역**:
1. **실운영 프로덕션 도메인 판별 및 데모 모드 원천 차단 (`src/services/demoMode.ts`)**:
   - `isProductionDomain()` 함수 신설: `ebro.run`, `giyuenlift.ebro.run`, `*.ebro.run`(demo 제외) 감지.
   - 실운영 도메인 감지 시 `localStorage.removeItem('ebro_demo_mode')`를 즉시 실행하고 무조건 `return false`를 반환하여 데모 모드 진입을 원천 봉쇄.
2. **로그인 화면 데모 버튼 100% 비노출 처리 (`src/App.tsx`)**:
   - `[⚡ 시연 데모 모드로 체험하기]` 버튼을 `isDemoMode()` 조건부 렌더링으로 감싸, **기연리프트 실운영 도메인에서는 0% 완전 비노출(삭제)**.
   - 오직 **`awp-demo.ebro.run` (데모 전용 도메인)** 접속 시에만 원클릭 체험 버튼 노출.
3. **도메인별 로그인 화면 브랜딩 자동 분기 (`src/App.tsx`)**:
   - **기연리프트 실운영 (`ebro.run`)**: 상단 기연리프트 공식 CI 로고 + `기연리프트` + `e-Bro ERP System` (데모 배너 및 데모 버튼 일체 없음).
   - **시연용 데모 (`awp-demo.ebro.run`)**: `ebro_awp` + `고소작업대 ERP 시연 데모` + `[⚡ 시연 데모 모드로 체험하기]` 노출.

**검증 결과**:
- 프로덕션 빌드: `npm run build` (`ebro_awp@0.0.0`) 1.75s 무오류 클린 통과.
