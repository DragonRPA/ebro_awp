...
const regex = /(const splitBillingAbsoluteAmount = async[^}]*\s*await saveDb\(db\);\s*\};\s*)/;
...
if (match) {
  const replacement = `const splitBillingAbsoluteAmount = async (billingId: string, splitAmount: number): Promise<void> => {
    ...
    db.insertRow('billings', childBilling);
...
`;
  ...
  console.log('Successfully fixed splitBillingAbsoluteAmount');
} else {
